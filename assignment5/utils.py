...
def pad_sents_char(sents, char_pad_token):
    ...
    for sent in sents:
        sent_padded = []
        word_pad_len = max(0, max_sentence_length - len(sent))
        for word in sent:
            char_pad_len = max_word_length - len(word)
            if char_pad_len >= 0:
                sent_padded.append(word + char_pad_len * [char_pad_token])
            else:
                sent_padded.append(word[:max_word_length])
        sent_padded.extend([pad_word for _ in range(word_pad_len)])
